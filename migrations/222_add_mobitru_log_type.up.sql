-- Ensure every project has the 'mobitru' system log type with the correct label color.
...
-- Fix mobitru log types created with the previous default label color.
UPDATE log_type
SET label_color = '#23A6DE',
    updated_at = now()
WHERE LOWER(name) = 'mobitru'
  AND level = 90000
  AND is_system = true
  AND label_color <> '#23A6DE';
