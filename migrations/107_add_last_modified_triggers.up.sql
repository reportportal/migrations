	...
	-- TEST_ITEM
	CREATE OR REPLACE FUNCTION update_last_modified_from_item()
	RETURNS TRIGGER AS $$
	BEGIN
	    UPDATE test_item
	    SET last_modified = CURRENT_TIMESTAMP
	    WHERE item_id = NEW.item_id OR item_id = OLD.item_id;

	    RETURN NEW;
	END;
	$$ LANGUAGE plpgsql;

	CREATE TRIGGER trg_update_test_item_last_modified_on_item
	AFTER INSERT OR UPDATE OR DELETE ON test_item
	FOR EACH ROW
	EXECUTE FUNCTION update_last_modified_from_item();
	...
	AFTER INSERT OR UPDATE OR DELETE ON test_item_results
	FOR EACH ROW
	...
	AFTER INSERT OR UPDATE OR DELETE ON launch
	FOR EACH ROW
